...
statepharmacy <- statepharmacy %>% 
  mutate(ratio = totalpharmacies / Population) %>% 
...
ggplot() +
  geom_sf(data = states, aes(fill = ratio)) + 
  #geom_text(data = states, aes(Longitude, Latitude, label = State), size = 2.5) +
  labs(x = "Longitude", y = "Latitude", title = "Ratio of Potential Pharmacies for COVID-19 Testing \n to Population Density per State") 
# + 
#   geom_text(data = sf_cent, aes(label = State, x = Longitude, y = Latitude, color = 'yellow'))
```
